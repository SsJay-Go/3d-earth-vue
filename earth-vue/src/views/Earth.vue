...
import CityMarkupPoint from '../components/CityMarkupPoint.vue'
...
<template>
    ...
    <TresGroup :ref="earthRef">
        ...
        <TresPoints>
            ...
            <TresPointsMaterial :size="0.01" color="#81ffff" :transparent="true" :opacity="0.1" :sizeAttenuation="true"
                :vertexColors="false" />
        </TresPoints>
        <!-- 创建地球辉光 -->
        <TresSprite :scale="[earthConfig.earth.radius * 3, earthConfig.earth.radius * 3, 1]">
            <!-- depthWrite: 渲染时不写入深度缓冲区-->
            <TresSpriteMaterial v-if="glowTexture" color="#4390d1" :map="glowTexture" :transparent="true" :opacity="0.2"
                :depthWrite="false" />
        </TresSprite>
        <!-- 创建地球大气层效果 -->
        <TresMesh>
        ...
        <!-- City Markup Point -->
         <TresGroup :ref="markupPointRef">
            <CityMarkupPoint v-for="item in citySite" :key="item" :item="item"/>
    ...
    <TresPoints>
        ...
        <TresPointsMaterial v-if="gradient" :size="2" color="#4d76cf" :transparent="true" :opacity="1"
            :sizeAttenuation="true" :map="gradient" />
    </TresPoints>
    <!-- 创建城市标识点 -->
    <TresSprite >
